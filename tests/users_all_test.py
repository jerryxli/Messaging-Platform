...
def test_one_user(clear_store, create_user):
    ...
    assert other.strip_url_image_profile(profile_response.json()['user']) == {"u_id": user_id, 'email': "z432324@unsw.edu.au", 'name_first': "Ji", 'name_last': "Sun", 'handle_str': "jisun"}
...
def test_multiple_users(clear_store, create_user, create_user2):
    ...
    assert [other.strip_url_image_profile(profile_response1.json()['user']), other.strip_url_image_profile(profile_response2.json()['user'])] == [   {"u_id": user_id, 'email': "z432324@unsw.edu.au", 'name_first': "Ji", 'name_last': "Sun", 'handle_str': "jisun"},
    ...
    response = requests.get(other.USERS_ALL_URL, params={'token': user_token})
    assert other.strip_array_url_image(response.json()['users']) == [{"u_id": user_id, 'email': "z432324@unsw.edu.au", 'name_first': "Ji", 'name_last': "Sun", 'handle_str': "jisun"}]
