...
def test_one_user(clear_store, create_user):
    ...
    response = requests.get(USERS_ALL_URL, json = {'token': user_token})
    profile_response = requests.get(PROFILE_URL, params = {"u_id": user_id}, json = {"token": user_token})
...
def test_multiple_users(clear_store, create_user, create_user2):
    ...
    response = requests.get(USERS_ALL_URL, json = {'token': user_token})
    profile_response1 = requests.get(PROFILE_URL, params = {"u_id": user_id}, json = {"token": user_token})
    profile_response2 = requests.get(PROFILE_URL, params = {"u_id": user_id2}, json = {"token": user_token2})
