...
def test_normal_profile(clear_store):
    ...
    profile_repsonse = requests.get(PROFILE_URL, params={"u_id": user_0_id}, json = {"token": user_0_token})
...
def test_invalid_user_id(clear_store):
    ...
    user_0_token = response.json()['token']
    user_0_id = response.json()['auth_user_id']
    profile_repsonse = requests.get(PROFILE_URL, params={"u_id": user_0_id + 1} , json = {"token": user_0_token})
