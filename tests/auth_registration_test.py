...
def test_auth_register_handle_generate(clear_store):
    ...
    profile_response_0 = requests.get(PROFILE_URL, params={"u_id": id_0, "token": token})
    assert profile_response_0.json()['user'] == {"u_id": id_0, "email":"z55555@unsw.edu.au", "name_first": "abcdefghi", "name_last": "jklmnopqrst", "handle_str": "abcdefghijklmnopqrst"}
    profile_response_1 = requests.get(PROFILE_URL, params={"u_id": id_1, "token": token})
    assert profile_response_1.json()['user'] == {"u_id": id_1, "email":"z55556@unsw.edu.au", "name_first": " **(&(abcdefghi__  ", "name_last": "jklmnopqrst&**uv", "handle_str": "abcdefghijklmnopqrst0"}
    profile_response_2 = requests.get(PROFILE_URL, params={"u_id": id_2, "token": token})
    assert profile_response_2.json()['user'] == {"u_id": id_2, "email":"z55557@unsw.edu.au", "name_first": "ABCDEFGHI", "name_last": "JKLMNOPQRSTUVWXYZ", "handle_str": "abcdefghijklmnopqrst1"}
