...
def check_user_left(user_token, user_id, dm_id):
    dm_details = requests.get(DM_DETAILS_URL, params = {'dm_id': dm_id}, json = {'token': user_token}).json()
    user_has_left = True
...
def test_not_a_member(clear_store, create_user, create_user2, create_user3):
    ...
    response = requests.post(DM_LEAVE_URL, json = {'token': user_token_3, 'dm_id': dm_id})
