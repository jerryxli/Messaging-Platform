...
def test_invalid_channel_id(clear_store, create_user):
    user_token_1 = create_user['token']
    response = requests.get(CHANNEL_MESSAGES_URL, params= {'channel_id': 1, 'start': 0}, json = {'token': user_token_1})
...
def test_user_not_in_channel(clear_store, create_user, create_user2):
    ...
    channel_id = requests.post(CREATE_URL, json={
                               'token': user_token_1, 'name': 'My Channel!', 'is_public': True}).json()['channel_id']
    response = requests.get(CHANNEL_MESSAGES_URL, params= {'channel_id': channel_id, 'start': 0}, json = {'token': user_token_2})
...
def test_channel_messages_start_exceeds(clear_store, create_user):
    ...
    channel_id = requests.post(CREATE_URL, json={
                               'token': user_token_1, 'name': 'My Channel!', 'is_public': True}).json()['channel_id']
    current = 0
    response = requests.get(CHANNEL_MESSAGES_URL, params={'channel_id': channel_id, 'start': current}, json = {'token': user_token_1})
    # Spew through the messages until we reach the end
    while response.json()['end'] != -1:
        current += 50
        response = requests.get(CHANNEL_MESSAGES_URL, params={'channel_id': channel_id, 'start': current}, json = {'token': user_token_1})
    response = requests.get(CHANNEL_MESSAGES_URL, params={'channel_id': channel_id, 'start': current + 100}, json = {'token': user_token_1})
    assert response.status_code == 400
...
def test_invalid_auth_id(clear_store, create_user):
    ...
    response = requests.get(CHANNEL_MESSAGES_URL, params={'channel_id': channel_id, 'start': 0}, json = {
                            'token': user_token_1})
    assert response.status_code == 403
