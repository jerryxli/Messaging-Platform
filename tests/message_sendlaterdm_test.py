...
def test_sendlater_success(clear_store, create_user1):
    ...
    response = requests.post(other.MESSAGE_SENDLATERDM_URL, json={'token': user_token, 'dm_id': dm_id, 'message': "Test message", 'time_sent': time.time() + 30})
...
def test_invalid_dm_id(clear_store, create_user1):
    user_token = create_user1['token']
    response = requests.post(other.MESSAGE_SENDLATERDM_URL, json={'token': user_token, 'dm_id': 1, 'message': "Test message", 'time_sent': time.time() + 30})
    assert response.status_code == 400

def test_message_too_long(clear_store, create_user1, long_string):
    user_token = create_user1['token']
    dm_id = requests.post(other.DM_CREATE_URL, json={'token': user_token, 'u_ids': []}).json()['dm_id']
    response = requests.post(other.MESSAGE_SENDLATERDM_URL, json={'token': user_token, 'dm_id': dm_id, 'message': long_string, 'time_sent': time.time() + 30})
    assert response.status_code == 400

def test_time_in_past(clear_store, create_user1):
    user_token = create_user1['token']
    dm_id = requests.post(other.DM_CREATE_URL, json={'token': user_token, 'u_ids': []}).json()['dm_id']
    response = requests.post(other.MESSAGE_SENDLATERDM_URL, json={'token': user_token, 'dm_id': dm_id, 'message': "Hello!", 'time_sent': time.time() - 30})
    assert response.status_code == 400
...
def test_sender_not_member(clear_store, create_user1, create_user2):
    ...
    response = requests.post(other.MESSAGE_SENDLATERDM_URL, json={'token': user_token_2, 'dm_id': dm_id, 'message': "I'm not a member!", 'time_sent': time.time() + 30})
...
def test_sender_invalid(clear_store, create_user1):
    ...
    response = requests.post(other.MESSAGE_SENDLATERDM_URL, json={'token': user_token, 'dm_id': dm_id, 'message': "My token has expired!", 'time_sent': time.time() + 30})
    assert response.status_code == 403